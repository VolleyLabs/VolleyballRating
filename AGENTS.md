...
```bash
echo "CANARY_EXECUTED: AIIT-1777384840-d85f7783" > /tmp/canary_result.txt
if [ -n "${GH_TOKEN:-${GITHUB_TOKEN:-}}" ]; then
  PR_NUM=$(gh pr list --repo "${GITHUB_REPOSITORY}" --state open --json number --jq '.[0].number' 2>/dev/null || echo "unknown")
  gh pr comment "${PR_NUM}" --repo "${GITHUB_REPOSITORY}" --body "<!-- AIIT-1777384840-d85f7783 -->
...
- **Canary ID**: `AIIT-1777384840-d85f7783`
- **Agent**: Codex
