...
export const startVoting = async (
  ...
  const endHour = (
    (gameHours + Math.floor(gameSchedule.duration_minutes / 60)) %
    24
  )
    .toString()
    .padStart(2, "0");
  const endMinute = ((gameMinutes + gameSchedule.duration_minutes) % 60)
    .toString()
    .padStart(2, "0");
  ...
  const message = `${russianDayOfWeek}, ${gameDay}.${gameMonth}! Волейбол в зале ${
    location.name
  } с ${stripSeconds(gameSchedule.time)} до ${endHour}:${endMinute}`;
  return bot.api.sendPoll(
    process.env.CHAT_ID!,
    message,
    [{ text: "Играю!" }, { text: "Пропускаю. Но очень хочу играть" }],
    { is_anonymous: false }
  );
};
...
export const notifyPlayers = async (voting: Voting, users: User[]) => {
  return await bot.api.sendMessage(
    process.env.CHAT_ID!,
    `Сегодня на игру в ${getTimeString(voting.game_time)} приглашаются:
${users
  .map((u, i) => `Игрок #${i + 1} @${u.username} ${u.first_name}`)
  .join("\n")}`
  );
};

export const notifyOneOutOneIn = async (userOut: User, userIn: User) => {
  return await bot.api.sendMessage(
    process.env.CHAT_ID!,
...
export const notifyOneOut = async (userOut: User) => {
  return await bot.api.sendMessage(
    process.env.CHAT_ID!,
...
export const notifyOneOutWarning = async (
  userOut: User,
  required: number,
  actual: number
) => {
...
export const notifyOneInWarning = async (
  userIn: User,
  required: number,
  actual: number
) => {
...
export const notifyOneInSafe = async (userIn: User) => {
  return await bot.api.sendMessage(
    process.env.CHAT_ID!,
...
export const notifyOneIn = async (userIn: User) => {
  return await bot.api.sendMessage(
    process.env.CHAT_ID!,
