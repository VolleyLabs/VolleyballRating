...
export default function RatingTable() {
  ...
  return (
    <div className={containerClasses} style={theme.cardBgStyle}>
      <div
      ...
      >
        ...
        <button
          ...
          disabled={refreshing}
          className={`p-2 rounded-full ${theme.primaryButton} ${
            refreshing ? "opacity-70" : theme.primaryButtonHover
          } transition-all`}
              ...
              : ratings.map((player, index) => (
                  <tr
                    ...
                    className={`border-t ${theme.tableBorder} ${
                      theme.tableRowHover
                    } ${player.username ? "cursor-pointer" : undefined}`}
                    style={theme.tableBorderStyle}
                    onClick={
                      player.username
                        ? () =>
                            window.open(
                              "https://t.me/" + player.username,
                              "_blank"
                            )
                        : undefined
                    }
