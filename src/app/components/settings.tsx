...
export default function Settings() {
  ...
  const [pickupHeight, setPickupHeight] = useState<number | undefined>(undefined);
  ...
  return (
    <div className="p-4">
      <h2 className={`text-xl font-bold ${theme.text} mb-4`} style={theme.textStyle}>
        User Settings
      </h2>
      
      <div className="mb-4">
        <label htmlFor="pickupHeight" className={`block text-sm font-medium ${theme.text} mb-1`} style={theme.textStyle}>
          Spike Height (cm)
        </label>
        <input
          type="number"
          id="pickupHeight"
          value={pickupHeight || ''}
          onChange={(e) => setPickupHeight(e.target.value ? Number(e.target.value) : undefined)}
          ...
          max="400"
        />
        <p className={`mt-1 text-xs ${theme.secondaryText}`} style={theme.secondaryTextStyle}>
